...
if (instance_exists(obj_enemy01)){
	if (distance_to_object(obj_enemy01) < 96){ //для врага 1
		var_fort_mode = "attack"
		if (var_fort_mode == "attack") {
			var_fort01_attackTime--
			if (var_fort01_attackTime < 0){
				instance_create_depth(x+16,y+16,-1,obj_fort03_projectile)
				var_fort01_attackTime = 50
			}
		}
...
if (instance_exists(obj_enemy04)){
	if (distance_to_object(obj_enemy04) < 96){ //для врага 4
		var_fort_mode = "attack"
		if (var_fort_mode == "attack") {
			var_fort01_attackTime--
			if (var_fort01_attackTime < 0){
...
if (instance_exists(obj_enemy02)){
	if (distance_to_object(obj_enemy02) < 96){ //для врага 1
		var_fort_mode = "attack"
		if (var_fort_mode == "attack") {
			var_fort01_attackTime--
			if (var_fort01_attackTime < 0){
